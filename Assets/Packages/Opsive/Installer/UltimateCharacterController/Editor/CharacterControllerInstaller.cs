...
namespace Opsive.UltimateCharacterController.Editor
{
    ...
    public class CharacterControllerInstaller : EditorWindow
    {
        ...
        private static void EditorStartup()
        {
            ...
            if (!EditorPrefs.GetBool("Opsive.UltimateCharacterController.Version3.Editor.InstallerWindowShown", false)) {
                EditorPrefs.SetBool("Opsive.UltimateCharacterController.Version3.Editor.InstallerWindowShown", true);
                ShowWindow();
            }
        ...
        private void OnGUI()
        {
...
#endif
            DrawRequirement("Unity 2021.3 or Newer", validUnityVersion);

            // If the user is updating from 2.x then a dialog should warn about the major changes.
            ...
            if (requireCleanInstall) {
                ...
                if (priorVersionInstalled) {
                    EditorGUILayout.HelpBox("Version 3.0 contains major structural changes. Before importing remove the Opsive/UltimateCharacterController and Opsive/Shared folders. " +
            ...
            GUILayout.Space(3);
            GUILayout.Label("Updates from versions prior to 3.0 require manual changes in order to upgrade your character.");
